...
class EventBot:
    ...
    async def process_phone_manual(self, message: types.Message, state: FSMContext):
        ...
        await state.update_data(phone=clean_phone)
        await message.answer("Введите ваше имя (можно указать до 3 слов, например: Анна или Иван Петров):")
        await state.set_state(RegistrationStates.waiting_for_full_name)
    ...
    async def show_menu(self, message: types.Message):
        ...
        if not events:
            keyboard = get_user_keyboard() if not self.is_admin(user_id) else get_admin_keyboard()
            await message.answer(
                "Пока что нет актуальных событий. Следите за сообщениями!",
...
class EventBot:
    ...
    def _register_handlers(self):
        handlers = [
            (self.cmd_start, Command(commands=['start'])),
            (self.cmd_menu, Command(commands=['menu'])),
            (self.cmd_admin, Command(commands=['admin'])),
            (self.handle_menu_button, F.text == "📖 Меню событий"),
            (self.handle_rocket_menu_button, F.text == "🚀 Вступить в ракету!"),
            (self.handle_admin_event_button, F.text == "➕ Добавить событие"),
            (self.handle_admin_rocket_button, F.text == "🚀 Вступить в ракету"),
            (self.handle_admin_broadcast_button, F.text == "✉️ Написать сообщение"),
            (self.handle_admin_stats_button, F.text == "📊 Статистика и выгрузки"),
            (self.handle_skip_button, F.text == "⏭️ Пропустить"),
            # Регистрация
            (self.process_consent, F.data == 'consent_yes', StateFilter(RegistrationStates.waiting_for_consent)),
            (self.process_phone, F.content_type == ContentType.CONTACT, StateFilter(RegistrationStates.waiting_for_phone)),
            (self.process_phone_manual, StateFilter(RegistrationStates.waiting_for_phone)),
            (self.process_full_name, StateFilter(RegistrationStates.waiting_for_full_name)),
            (self.process_events_choice, F.data.startswith('send_events_'), StateFilter(RegistrationStates.waiting_for_events_choice)),
            # Админка — события
            (self.process_event_title, StateFilter(AdminStates.waiting_for_event_title)),
            (self.process_event_description, StateFilter(AdminStates.waiting_for_event_description)),
            (self.process_event_photo, F.content_type == ContentType.PHOTO, StateFilter(AdminStates.waiting_for_event_photo)),
            (self.skip_event_photo, StateFilter(AdminStates.waiting_for_event_photo)),
            # Админка — ракета
            (self.process_rocket_title, StateFilter(AdminStates.waiting_for_rocket_title)),
            (self.process_rocket_description, StateFilter(AdminStates.waiting_for_rocket_description)),
            (self.process_rocket_photo, F.content_type == ContentType.PHOTO, StateFilter(AdminStates.waiting_for_rocket_photo)),
            (self.skip_rocket_photo, StateFilter(AdminStates.waiting_for_rocket_photo)),
            # Редактирование ракеты
            (self.process_edit_rocket_title, StateFilter(AdminStates.waiting_for_edit_rocket_title)),
            (self.process_edit_rocket_description, StateFilter(AdminStates.waiting_for_edit_rocket_description)),
            (self.process_edit_rocket_photo, F.content_type == ContentType.PHOTO, StateFilter(AdminStates.waiting_for_edit_rocket_photo)),
            (self.skip_edit_rocket_photo, StateFilter(AdminStates.waiting_for_edit_rocket_photo)),
            # Рассылка
            (self.process_broadcast_message, StateFilter(AdminStates.waiting_for_broadcast_message)),
            (self.process_broadcast_photo, F.content_type == ContentType.PHOTO, StateFilter(AdminStates.waiting_for_broadcast_photo)),
            (self.skip_broadcast_photo, StateFilter(AdminStates.waiting_for_broadcast_photo)),
            (self.process_broadcast_target, F.data.startswith('target_'), StateFilter(AdminStates.waiting_for_broadcast_target)),
            # Статистика
            (self.handle_stats_export_users, F.data == "stats_export_users"),
            (self.handle_stats_export_applications, F.data == "stats_export_applications"),
            (self.handle_stats_show_events, F.data == "stats_show_events"),
            (self.handle_event_export, F.data.startswith('event_export_')),
            (self.handle_event_delete, F.data.startswith('event_delete_')),
            # Рассылка по событию
            (self.start_custom_broadcast_for_event, F.data.startswith('event_custom_broadcast_')),
            (self.process_custom_broadcast_text, StateFilter(AdminStates.waiting_for_custom_event_broadcast)),
            (self.process_custom_broadcast_photo, F.content_type == ContentType.PHOTO, StateFilter(AdminStates.waiting_for_custom_event_photo)),
            (self.skip_custom_broadcast_photo, StateFilter(AdminStates.waiting_for_custom_event_photo)),
            # Редактирование события
            (self.process_event_edit_choice, F.data.startswith('event_edit_')),
            (self.process_edit_title, StateFilter(AdminStates.waiting_for_edit_title)),
            (self.process_edit_description, StateFilter(AdminStates.waiting_for_edit_description)),
            (self.process_edit_photo, F.content_type == ContentType.PHOTO, StateFilter(AdminStates.waiting_for_edit_photo)),
            (self.skip_edit_photo, StateFilter(AdminStates.waiting_for_edit_photo)),
            # Удаление
            (self.confirm_delete_event, F.data.startswith('confirm_delete_'), StateFilter(AdminStates.waiting_for_delete_confirmation)),
            (self.cancel_delete_event, F.data.startswith('cancel_delete_'), StateFilter(AdminStates.waiting_for_delete_confirmation)),
            # Заявки
            (self.handle_apply, F.data.startswith('apply_')),
            (self.handle_rocket_apply, F.data == 'rocket_apply'),
            # Просмотр
            (self.handle_view_event, F.data.startswith("view_event_")),
            (self.handle_admin_view_event, F.data.startswith("admin_view_event_")),
            (self.handle_event_resend_all, F.data.startswith("event_resend_all_")),
        ]
        for handler in handlers:
            if len(handler) == 2:
                self.router.message.register(handler[0], handler[1])
            elif len(handler) == 3:
                if 'callback' in str(handler[0]):
                    self.router.callback_query.register(handler[0], handler[1], handler[2])
                else:
                    self.router.message.register(handler[0], handler[1], handler[2])
    ...
    # === Все методы из твоего файла (без изменений, кроме использования self.db) ===
    # (ниже вставлены все методы как в Pasted_Text_1769504465429.txt)
