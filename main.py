...
class EventBot:
    ...
    def _register_handlers(self):
        ...
        # Рассылка по событию с кастомным описанием (название берётся из БД)
    ...
    async def _send_long_message_with_photo_and_button(self, chat_id, text, photo_id, reply_markup):
        """Отправляет длинное сообщение: сначала текст (без кнопки), потом фото с кнопкой и подписью '📎'."""
    ...
    async def process_full_name(self, message: types.Message, state: FSMContext):
        ...
        if not re.match(r'^[А-Яа-яЁё\s\-]{3,}$', display_name):
            ...
            return
        await state.update_data(full_name=display_name, user_name=display_name)
        ...
        if choice == "send_events_yes":
            ...
            if events:
                ...
                for event in events:
                    ...
                    if photo_id:
                        try:
                            await self._send_long_message_with_photo_and_button(callback_query.message.chat.id, caption, photo_id, keyboard.as_markup())
                        except Exception as e:
                ...
                await callback_query.message.answer("Пока что нет актуальных событий. Следите за сообщениями!")
        keyboard = get_user_keyboard() if user_id != ADMIN_ID else get_admin_keyboard()
    ...
    async def show_menu(self, message: types.Message):
        ...
        for event in events[:3]:
            ...
            if photo_id:
                try:
                    await self._send_long_message_with_photo_and_button(message.chat.id, caption, photo_id, keyboard.as_markup())
                except Exception as e:
                    logging.error(f"Ошибка отправки фото события: {e}")
    ...
    async def _send_rocket_broadcast(self, message, state):
        ...
        await message.answer("Выберите кому отправить информацию о вступлении в ракету:", reply_markup=keyboard)
    ...
    async def handle_stats_show_events(self, callback_query: types.CallbackQuery):
        ...
        for event in events:
            ...
            keyboard.button(text=f"✏️ {title}", callback_data=f"event_edit_{event_id}")
            keyboard.button(text=f"📤 {title}", callback_data=f"event_custom_broadcast_{event_id}")
        keyboard.adjust(2)
    ...
    async def _send_custom_event_broadcast(self, message: types.Message, state: FSMContext):
        ...
        # Формируем текст с названием события и отступами
