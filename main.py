...
class EventBot:
    ...
    async def process_full_name(self, message: types.Message, state: FSMContext):
        ...
        display_name = " ".join(words[:3])  # максимум 3 слова

        # Проверка: только русские буквы, пробелы, дефисы; минимум 3 символа
        ...
        phone = data['phone']
        self.db.add_user(user_id, display_name, phone, username)  # gender=None, birth_date=None
    ...
    async def handle_resend_choice(self, callback_query: types.CallbackQuery, state: FSMContext):
        ...
        if choice == "resend_same":
            ...
            await callback_query.message.answer("Выберите аудиторию для повторной рассылки:", reply_markup=keyboard)
    ...
    async def _save_and_send_event(self, message, state):
        ...
        keyboard = self._create_target_selection_keyboard("event")
        await message.answer("Выберите кому отправить мероприятие:", reply_markup=keyboard)
